...
class FixedSizeChunker(BaseChunker):
    ...
    overlap_tokens — перекрытие между соседними чанками.
    """
...
class StructuralChunker(BaseChunker):
    ...
    Разбивает текст по структуре документа:
      ...
      - Python    → по определениям классов и функций (AST)
      - Остальное → по абзацам (двойной перенос строки)

    Слишком большие секции дробятся FixedSizeChunker'ом (fallback).
    """
    ...
    def _split_markdown(text):
        """Разбить Markdown по заголовкам."""
    ...
    def _split_python(text):
        """Разбить Python-код по классам и функциям (top-level)."""
        try:
            tree = ast.parse(text)
        except SyntaxError:
        ...
        for node in ast.iter_child_nodes(tree):
            if isinstance(node, (ast.FunctionDef, ast.AsyncFunctionDef, ast.ClassDef)):
                ...
                # Текст между предыдущей секцией и этой
