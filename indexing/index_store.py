...
class IndexStore(BaseStore):
    ...
    def add_chunks_batch(self, chunks, embeddings_bytes=None):
        """Добавить пакет чанков. embeddings_bytes — список bytes или None."""
        ids = []
    ...
    def get_all_chunks(self, strategy=None, source_file=None):
        """Получить все чанки с фильтрацией."""
        query = "SELECT id, source_file, title, strategy, chunk_index, " \
                "start_offset, token_count, text, metadata_json FROM chunks WHERE 1=1"
        params = []
        if strategy:
            query += " AND strategy = ?"
    ...
    def get_embedding(self, chunk_id):
        """Получить эмбеддинг (bytes) по chunk_id."""
        row = self.conn.execute(
    ...
    def count_chunks(self, strategy=None):
        """Кол-во чанков (опционально по стратегии)."""
        if strategy:
            row = self.conn.execute(
